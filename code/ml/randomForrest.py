		...
		if domain.endswith(".csv"):
			...
			if domainMapping != None:
				...
				if len(t) > cm.n_threshold:
					...
					cnt += 1
	if domainMapping != None:
		print("Not found domain = ",len(NotFoundDomain))
...
def main(args=0):
	...
	if args.version != None:
		...
		x1,y1 = ReadAllFeatures(inputdir1.strip(),args.outputdir)
		print("len x1 = ",len(x1))
		dirpath = os.path.join(args.inputdir,"mlresult/%s-%s/"%(v1,v2))
		...
		Testing(clf,x1_test,y1_test,args.outputdir)
		Testing(clf,x2_test,y2_test,args.outputdir)
		VisualizeFeatures(clf,args.outputdir)
	else:
		x,y = ReadAllFeatures(args.inputdir,args.outputdir)
		print("finish reading all features...")
